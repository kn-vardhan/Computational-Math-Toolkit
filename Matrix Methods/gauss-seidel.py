...
def gauss_seidel(X: list(float), A: list(float), B: list(float), tolerance: float) -> list(float):
    """
    Solves a system of linear equations using Gauss-Seidel Iterative Method.

    Args:
        X (list(float)): Initial guess matrix.
        A (list(float)): Coefficient matrix.
        B (list(float)): Constant matrix.
        tolerance (float): Error tolerance.

    Returns:
        list(float): Approximate solution of the given system of linear equations.
    """

    # Converting the input parameters to numpy arrays
    X = np.array(X)
    A = np.array(A)
    B = np.array(B)
    
    # Performing Gauss-Seidel Iterative Method until error tolerance is achieved
    while True:
        gauss_seidel.iterations += 1
        ...
        if gauss_seidel.iterations == MAX_ITERS:
            break
...
if __name__ == '__main__':
    ...
    start = time.time()
    gauss_seidel.iterations = 0
    final_X = gauss_seidel(init_mat, matrix_A, matrix_B, error)
    end = time.time() - start
    ...
    print(f'Iterations to achieve the error tolerance = {gauss_seidel.iterations}')
